...
# =========================================================
# Layout Logic (Hardcoded Spacing + Block Centering)
# =========================================================
def calculate_layout_params(sheet_length, sheet_width, item_size, spacing, pattern_type):
    
    # 1. HARDCODED LOGIC FOR SLOTS (Exactly as requested)
    if pattern_type == "slot":
        SLOT_L = 40.0
        SLOT_H = 8.5
        GAP_X = 8.5        # Hardcoded horizontal gap
        PITCH_Y = 25.0     # Hardcoded vertical pitch
        PITCH_X = SLOT_L + GAP_X
        STAGGER = PITCH_X / 2
        
        # Calculate how many slots fit in an EVEN row (with at least 18mm margins)
        # Even row width: SLOT_L + (count-1)*PITCH_X
        # Odd row width (with stagger): SLOT_L + (count-1)*PITCH_X + STAGGER
        # We solve for the wider row (Even Row + Stagger) to protect the right margin
        count_x = math.floor((sheet_length - 36 - SLOT_L - STAGGER) / PITCH_X) + 1
        
        # Total Block Width is defined by the Even Row + Stagger shift
        total_block_w = SLOT_L + (count_x - 1) * PITCH_X + STAGGER
        margin_x = (sheet_length - total_block_w) / 2
        
        # Vertical Packing
        count_y = math.floor((sheet_width - 36 - SLOT_H) / PITCH_Y) + 1
        total_block_h = SLOT_H + (count_y - 1) * PITCH_Y
        margin_y = (sheet_width - total_block_h) / 2
    ...
    # 2. STANDARD LOGIC FOR OTHER PATTERNS (Squares, Circles, Diamonds)
    ...
    return {
        ...
        "pitch_y": pitch_y,
        "stagger": pitch_x / 2,
        "margin_x": (sheet_length - (item_size + (count_x-1)*pitch_x)) / 2,
        "margin_y": (sheet_width - (item_size + (count_y-1)*pitch_y)) / 2,
        "hole_w": item_size,
        "hole_h": item_size
    }
...
async def generate_dxf(payload: dict = Body(...)):
    ...
    # Calculate layout using the hardcoded logic for slots
    ...
    for row in range(layout["count_y"]):
        ...
        is_odd = (row % 2 != 0)
        row_offset = layout["stagger"] if is_odd else 0
        
        # For slots, odd rows have one fewer slot to protect the right margin
